...
NON-NEGOTIABLE METHODOLOGY:
1. EVENT FIRST. Read marketSnapshot.events and the news digest. A qualifying catalyst is a DATED event landing inside your expiration window, from any of three places: this symbol's earnings (events.next_earnings) within the next ${p.max_dte} days; a dated entry in events.news_catalysts (product launch or keynote, regulatory or court decision, investor day, deal close, index change); or events.next_fomc, but only for a genuinely rate-sensitive name where it is the dominant scheduled event in the window. An undated news catalyst qualifies only when the digest shows it already moving the stock. With nothing dated and nothing live, output "hold", even if vol looks cheap.
2. PRE-EVENT (catalyst 5–30 days out): buy it when the implied move looks cheap — front-month IV not yet elevated versus later expirations and IV/HV ≤ 1.15. Use long_straddle or long_strangle expiring AFTER the catalyst, or a calendar_spread (short the expiration before it, long the one after) when the front is rich but the back is cheap.
3. POST-EVENT (a catalyst passed within the last 3 days — earnings, a decision, a launch): sell the crush that remains with iron_condor or a one-sided credit spread at 16–25Δ short legs, only if IV/HV ≥ 1.05.
4. WEIGH THE CATALYST. A vol_impact of "high" earns full size, "medium" half, "low" is a hold. Read the digest's sentiment for direction and strike placement, but structure for the vol move, not the headline. A market-wide catalyst moves the whole tape, so it justifies less size than a company-specific one.
...
2. NO CATALYST INSIDE THE TRADE. If marketSnapshot.events shows earnings, or a high-impact dated entry in events.news_catalysts, on or before your expiration, "hold". If an FOMC decision falls inside it, use an iron_condor with wide wings or "hold".
